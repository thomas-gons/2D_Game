...
void player_update() {
    ...
    case UP:
        ...
        break;
    // TODO: Diagonal checks, adding distances values
    default: break;
    }
}
...
bool player_check_collisions(uint8_t line, uint8_t col) {
    ...
    if (IS_OUT_OF_MAP(line, col)) {         
        ...
        if (IS_OBSTACLE_CELL(line, col)) {
            ...
            // Update player position in map according to move
...
void player_obstacle_alert(uint8_t line, uint8_t col) {
    ...
    mvwprintw(game.game_win, line, col, OBSTACLE_CHAR);
        ...
        if (index > 0) {
            ...
            for (int16_t i = 0; i < index; i++) {
                ...
                case ROAD:
                    ...
                    break;
                case FRUIT:
                    mvwaddch(game.game_win, tmp->pos.l, tmp->pos.c, FRUIT_CHAR | COLOR_PAIR(FORMAT_COLOR_MAGENTA));
                    break;
                case NO_FRUIT:
...
void player_stack_fruit(uint8_t line, uint8_t col) {
    if (player->stamina >= STAMINA_LIMIT_TO_STACK_FRUITS && player->fruit_stack < FRUIT_STACK_MAX) {
...
void player_stats_render() {
    // Render fruit stack
    mvwprintw(game.stats_win, 2, 3, "FRUITS");
    mvwprintw(game.stats_win, 2, 12, ".  .");
    for (uint8_t i = 0; i < player->fruit_stack; i++) {
        wattron(game.stats_win, A_BOLD);
        mvwaddch(game.stats_win, 2, 12 + i * 3, FRUIT_CHAR | COLOR_PAIR(FORMAT_COLOR_GREEN));
...
void player_render() {
    ...
    mvwprintw(game.game_win, player->pos.l, player->pos.c, PLAYER_CHAR);
